...
export const BookingForm = ({ motionForm }) => {
  const [captchaStatus, setCaptchaSatus] = useState(null);
  ...
  function handleCaptcha(value) {
    if (value) {
      setCaptchaSatus(true);
    } else {
      setCaptchaSatus(false);
    }
  }
  ...
  return (
    <>
      ...
      <span>
        {captchaStatus === false && (
          <p className="error-message">
            Oops looks like the recaptcha check failed! Please try again.
          </p>
        )}
      </span>
      ...
      {validationMessage.successMessage ? (
        ...
        <motion.div
          className={loadingForm ? "form submitting" : "form"}
          variants={motionForm}
        >
          <form
            name="contact"
            ...
            className={loadingForm ? "form-body submitting" : "form-body"}
          >
            <div
              className={
              ...
              {booking.bookStudio && (
                <motion.div variants={motionForm} className="form-inner">
                  <span className="form__cancellation-notice">
                    *Cancellation must be made no later than 5 days before the
                    booked date, otherwise we have the right to charge half of
                    the agreed amount.
                  </span>
