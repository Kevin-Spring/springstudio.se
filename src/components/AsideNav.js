...
export const AsideNav = ({ posts, loading, sections /* , changeAutoKill */ }) => {
  ...
  useEffect(() => {
    ...
    //cleanup to prevent eventlisteners do be added multiple times
    return () => {
      refDots.current.forEach((dot, index) => {
      ...
      <aside className='dots-container'>
        <nav>
          <ul>
            {posts.map((post, i) => {
              return (
                <li key={post.id} /* onClick={() => changeAutoKill(true)} */>
                  <a ref={addToRefDots} href={`#section${i + 1}`}>
                    <Dots key={post.id} id={post.id} title={post.title.rendered} sections={sections} />
                  </a>
                </li>
              )
            })}
